...
(function($) {
  $.fn.agenda = function( options ) {
    ...
    var start_orig = this.children().first().data('start');
    var end_orig = this.children().last().data('end');

    var offset_duration = moment.duration("00:30:00");

    // calculation of 30min-offset
    var start = moment( start_orig, "HH:mm" ).subtract(offset_duration);
    var end = moment( end_orig, "HH:mm" ).add(offset_duration);

    // var agenda_duration = moment.duration(end.diff(start, 'hours'));
    var agenda_duration = (end.diff(start, 'hours') + 1);


    var html = [];
    for (var i = 0; i <= agenda_duration; i++) {
      var time = start;
      var hour = time.startOf('hour').format('HH:mm').toString();
      var plain = time.startOf('hour').format('HH').toString();
      html += '<div class="schedule_hour sans-serif" data-agenda="' + plain + '">'
        + '<div class="schedule_hour_time">'
          + '<p class="white">' + hour + '</p>'
        + '</div>'
      + '</div>'
    ...
    // var agenda_height = 0;
    // $('.schedule_hour').each( function() {
    //   var height = $(this).outerHeight(true)
    //   agenda_height = agenda_height + height;
    // });

    var agenda_first = start;
    var agenda_last = agenda_first.add(agenda_duration, 'hours');
    //console.log(agenda_first.startOf('hour').format('HH:mm').toString());


    function calculate_el( element ) {
      //var this = element;
      ...
      //     });
      //   }
      // } else if ( start_hours + 1 == end_hours ) {
      //   // only start and stop
